...
classdef OUT_regridded2 < matlab.mixin.Copyable
    ...
    methods
        ...
        function out = finalize_init(out, tile)
            ...
            out.display_step(tile);

            forcing = tile.FORCING;
            
            % Set the next (first) output time. This is the next (first) time output is collected (in memory) for later storage to disk.
            out.OUTPUT_TIME = forcing.PARA.start_time + out.PARA.output_timestep;
            %make struct "result" and initialize all variables defined by the user as empty arrays
            out.timestamp = [];
        ...
        function display_step(out, tile)
            t = tile.t;
            step = out.PARA.display_timestep;
            
            if isempty(out.next_display_step)
                out.next_display_step = t;
            end

            if t >= out.next_display_step
                tile_number = tile.PARA.class_index;
                run_number = tile.PARA.range;
                fprintf('[g%04d : t%d] %s\n', run_number, tile_number, datestr(t));

                out.next_display_step = out.next_display_step + step;
            end
        end
